    ...
    public sealed class MemCache<K, V> : IMemCache<K, V>
    {
        ...
        public V GetOrUpdate<A0, A1>(K key, Func<K, V, A0, A1, V> func, A0 arg0, A1 arg1)
        {
            ...
            try
            {
                ...
                Interlocked.Increment(ref MissCount);
                var e = func(key, val.Item2, arg0, arg1);
                var exp = GetExpirationTimeUtc(e);
                val = ValueTuple.Create(exp, e, (Task<V>)null);
                c[key] = val;
                UpdatePrio(key, exp);
        ...
        public async Task<V> GetOrUpdateAsync(K key, Func<K, V, Task<V>> func, bool waitUntilReady = true)
        {
            ...
            try
            {
                ...
                if (waitUntilReady)
                {
                    var e = await func(key, val.Item2).ConfigureAwait(false);
                    var exp = GetExpirationTimeUtc(e);
                    val = ValueTuple.Create(exp, e, (Task<V>)null);
                    c[key] = val;
                    UpdatePrio(key, exp);
                ...
                else
                {
                    async Task<V> build()
                    {
                        ...
                        try
                        {
                            e = await func(key, val.Item2).ConfigureAwait(false);
                        }
                        catch
                        {
        ...
        public async Task<V> GetOrUpdateAsync<A>(K key, Func<K, V, A, Task<V>> func, A arg, bool waitUntilReady = true)
        {
            ...
            try
            {
                ...
                if (waitUntilReady)
                {
                    var e = await func(key, val.Item2, arg).ConfigureAwait(false);
                    var exp = GetExpirationTimeUtc(e);
                    val = ValueTuple.Create(exp, e, (Task<V>)null);
                    c[key] = val;
                    UpdatePrio(key, exp);
                ...
                else
                {
                    async Task<V> build()
                    {
                        ...
                        try
                        {
                            e = await func(key, val.Item2, arg).ConfigureAwait(false);
                        }
                        catch
                        {
                            c.TryRemove(key, out var _);
        ...
        public async Task<V> GetOrUpdateAsync<A0, A1>(K key, Func<K, V, A0, A1, Task<V>> func, A0 arg0, A1 arg1, bool waitUntilReady = true)
        {
            ...
            try
            {
                ...
                if (waitUntilReady)
                {
                    var e = await func(key, val.Item2, arg0, arg1).ConfigureAwait(false);
                    var exp = GetExpirationTimeUtc(e);
                    val = ValueTuple.Create(exp, e, (Task<V>)null);
                    c[key] = val;
                    UpdatePrio(key, exp);
                ...
                else
                {
                    async Task<V> build()
                    {
                        ...
                        try
                        {
                            e = await func(key, val.Item2, arg0, arg1).ConfigureAwait(false);
                        }
                        catch
                        {
                            c.TryRemove(key, out var _);
