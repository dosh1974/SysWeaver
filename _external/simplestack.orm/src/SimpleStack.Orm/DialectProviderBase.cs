...
namespace SimpleStack.Orm
{
    ...
    public abstract class DialectProviderBase : IDialectProvider
    {
        ...
        /// <inheritdoc />
        public virtual CommandDefinition ToSelectStatement(SelectStatement statement, CommandFlags flags, CancellationToken cancellationToken = new CancellationToken(), int? commandTimeout = null)
        {
            ...
            if (!String.IsNullOrEmpty(statement.As))
                sql.Append(" AS ").Append(statement.As);
            ...
            var cmd = sql.ToString();
            return new CommandDefinition(cmd, statement.Parameters.ToDynamicParameters(), flags: flags, cancellationToken:cancellationToken, commandTimeout: commandTimeout);
        }
        ...
        public virtual CommandDefinition ToCreateTableStatement(ModelDefinition modelDef, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            if (sbPrimaryKeys.Length > 0)
            {
                ...
                if (!String.IsNullOrEmpty(modelDef.PrimaryKeyType))
                    sbPrimaryKeys.Append(" USING ").Append(modelDef.PrimaryKeyType).Append(' ');
            }
            ...
            StatementParameters parameters, params string[] availableParameters)
        {
            switch (functionName.ToLower())
            {
                ...
                case "contains":
                    ...
                    return $"upper({quotedColumnName}) LIKE {availableParameters[0]} ";
                case "compareto":
                    return $"strcmp({quotedColumnName},{availableParameters[0]})";
                case "substring":
...
#pragma warning restore 618
        ...
        /// <param name="tableName">Optionally override the table name</param>
        /// <returns>The model.</returns>
        protected static ModelDefinition GetModel(Type modelType, String tableName = null)
        {
            return modelType.GetModelDefinition(tableName);
        }
        ...
        public static ModelDefinition GetModelDefinition(Type modelType, String tableName = null)
        {
            return modelType.GetModelDefinition(tableName);
        }
