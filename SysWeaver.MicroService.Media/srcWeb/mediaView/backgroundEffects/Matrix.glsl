...
const vec3 Color = vec3(0.05, 0.15, 0.10);//	var: { "type": "colhdr" }
const float TextOpacity = 0.25; // var: {}

float rand(float x) { return fract(sin(x * 1232.0) * 4358.5453123); }
...
{
    vec2 p = fragCoord.xy * 0.5;
	vec2 uv = p / iResolution.xy - 0.5;
    float col = p.x / 8.0;
    col = rand(col- fract(col)) * 40.0 - 5.0;
    ...
    float a = -0.3*smoothstep(0.1, 0.8, length(uv)) + 
        invader(ip, 809999.0*r) * (0.06 + 0.15*ring(uv,r) + max(0.0, 0.2*sin(10.0*r*iTime)));
    a *= TextOpacity;
