...
const float Scale = 2.0;//var:{}
const vec3 Color = vec3(0.5,0.95, 1.0);// var: { "type": "colhdr" }


void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    float time = iTime * 0.05 + 32.0;
    vec2 p = Scale *(( fragCoord.xy-.5* iResolution.xy )/iResolution.y)-.5 + vec2(2.0, 1.0);
    vec2 i = p;
	float c = 0.0;
	float r = length(p+vec2(sin(time),sin(time*.222+99.))*1.5);
	float d = length(p);
	float rot = d+time+p.x*.915; 
	for (float n = 0.0; n < 7.0; n++) {
		p *= mat2(cos(rot-sin(time/1.)), sin(rot), -sin(cos(rot)-time), cos(rot))*-0.75;
	...
	c -= 0.35;
	c = max(c, 0.0);
	fragColor = PostProcess(vec4(Color * c, 1.0), fragCoord.xy / iResolution.xy);
}
