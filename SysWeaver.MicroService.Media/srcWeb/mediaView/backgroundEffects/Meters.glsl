...
const float Radius = 1.0; 			// var:	{ "min": 0, "max": 1, "step": 0.05, "desc": "The relative corner radius"}
const float BarWidth = 64.0; 		// var:	{ "min": 1, "max": 256, "step": 1, "desc": "The width in pixels of each bar (including spacing)"}
const float BarFill = 0.95; 		// var:	{ "min": 0.01, "max": 1.00, "step": 0.01, "desc": "How much of the allocated space that a bar should fill"}
...
const int Palette = 0;				// var:	{ "min": 0, "max": 4, "step": 1, "desc": "What palette to use"}
...
const float GradientCompress = 0.2; // var:	{ "min": 0, "max": 1, "step": 0.05, "desc": "How much to compress the gradient when the bar compresses"}
...
const float Attach = 0.0;			// var: { "min": 0.0, "max": 1.0, "step": 0.05, "desc": "Where to attach the bars, 0 = bottom, 0.5 = center, 1.0 = top"} 
...
vec2 GetBarIntensity(vec2 relPos, float barIndex, float time)
{
	...
	compY /= mix(height, iResolution.y, GradientCompress);
	return vec2(i, compY);
}
...
{    
	...
	vec4 color = mix(BgColorBottom, BgColorTop, uv.y);
	color.rgb *= color.w;
