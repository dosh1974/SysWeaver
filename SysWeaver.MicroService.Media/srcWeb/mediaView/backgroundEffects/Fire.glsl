...
const vec4 BgColor = vec4(0.01, 0.01, 0.03, 1.0); //var:{}
const float Clip = 210.0; //var:{}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	//
	float xfuel =2.0-abs(2.0*xpart-1.0);//pow(1.0-abs(2.0*xpart-1.0),0.5);
	//
	float timeSpeed = 0.5;
	...
	float fff = f*f*f;
	vec4 fire = 1.5*vec4(f, fff, fff*fff,fff);
	//
	// smoke
	float smokeNoise = 0.5+snoise(0.4*position+timing*vec3(1.0,1.0,0.2))/2.0;
	float smokeAlpha = 0.3*pow(xfuel,3.0)*pow(ypart * 1.2 + 0.5,2.0)*(smokeNoise+0.4*(1.0-noise));
	//
	// sparks
	...
	if (sparkLife>0.0) {
		...
		sparks = sparkLife*sparksGray*vec4(1.0,0.3,0.0,1.0);
	}
	//

	fire += sparks;
	
	float additive = clamp(fire.w, 0.0, 1.0);
	
	vec4 fireCol = vec4(vec3(fire) * additive, additive);
	
	smokeAlpha = clamp(smokeAlpha * 2.75 - 0.5,0.0,1.0) * 0.95;
	smokeNoise = (noise * 0.02 + smokeNoise * 0.1 + 0.01) * smokeAlpha;
	vec4 smoke = vec4(smokeNoise, smokeNoise, smokeNoise, smokeAlpha);
	

	vec4 col = blend(smoke, fireCol);
	
	col = blend(BgColor, col);
	//col = vec4(col.w, col.w, col.w, 1.0);
	fragColor = PostProcess(col, uv);
	//vec4(max(fire,sparks)+smoke);
}