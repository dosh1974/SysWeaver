...
const vec4 Color = vec3(0.1, 0.6, 1.0, 1.0);//	var: { "type": "colhdr" }
const int LineCount = 20;//var:
...
const float NoiseInt = 0.25; 	//	var: { "min": 0, "max": 1.0, "step": 0.05, "desc": "The amount of noise, set to 0 to disable"}
...
void main( void ) {
	vec2 uv = (gl_FragCoord.xy - resolution * vec2(0.9, 1.0)) / min(resolution.x, resolution.y);
	uv *= 0.5;
	uv += vec2(0.1, 0.0);
	float t = time * 0.1 + 20.0;
	vec2 uvs = vec2(12.4, 7.5) * uv;
	float e = 0.0;
	for (int i=0;i<LineCount;++i) {
		float ii = float(i + 3);
		vec2 is = vec2(0.015, 0.75) * ii;
		e += 0.2/abs(2.5 - (ii/25.) + sin(t + is.x*uvs.x * cos(is.y + t + uvs.x)) + uvs.y);
	}
	e /= float(LineCount);
	e = sqrt(e);
	...
	gl_FragColor = PostProcess(color, gl_FragCoord.xy / resolution.xy);
