...
vec2 hexID(vec2 p){ return floor(vec2(p.x - p.y*s.x*2./3., p.y/s.x)); }

// Different from "sign" in that it has no zero value.
float sgn(float x){ return x<0.? -1. : 1.; }
 
 
 // Environment — Pitch black background, pure white studio lights
vec3 envMap(vec3 rd) {
    vec3 col = vec3(0.01); 
...
void mainImage( out vec4 fragColor, vec2 fragCoord){
    ...
    for(int i=0; i<N*N; i++){ 
		...
		float fi = float(i);
        // Offset ID.
        vec2 idOffs = ip + vec2(mod(fi, NI), float(i / N)) - float(N - 1)/2.;
    ...
    if(t<1e5){ 
        ...
        vec3 txR = envMap(reflect(rd, n)); txR *= txR;
