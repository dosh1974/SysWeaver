...
namespace SysWeaver.ExchangeRate.Sources
{
    ...
    public sealed class ErsExchangeRatesApiDotIo : IRateSource, IDisposable
    {

        public ErsExchangeRatesApiDotIo(ErsExchangeRatesApiDotIoParams p)
        {
            P = p;
        ...
        readonly ErsExchangeRatesApiDotIoParams P;
...
#pragma warning restore
        ...
        public async Task<Rates> GetRates()
        {
            var b = Reference;
            var key = P.GetApiKey();
            var url = String.Concat("https://api.exchangeratesapi.io/v1/latest?access_key=", key, "&base=", b);
            byte[] data = await Client.GetByteArrayAsync(url).ConfigureAwait(false);
            var d = S.Create<Data>(data.AsSpan());
            if (!d.success)
            {
                var e = d.error?.message;
